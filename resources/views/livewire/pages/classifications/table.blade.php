            ...
            <div class="small-box shadow-none">
                ...
                <div class="table-responsive rounded">
                    ...
                    <table class="table table-borderless table-hover">
                        <caption><small>Classificações cadastradas <b>{{$data->count()}}</b></small></caption>
                        ...
                        @foreach($data as $d)
                            <tr>
                                <td>{{$d->id}}</td>
                                <td>{{$d->name}}</td>
                                <td class="text-center">{{$d->months}}</td>
                                <td class="text-center">{{$d->updated_at}}</td>
                                <td class="text-center">
                                    @if($d->deleted_at === null)
                                        <a href="{{route('classifications.edit', $d->id)}}"
                                           class="btn btn-primary btn-sm tooltips" data-text="Editar">
                                            <i class="fas fa-sync-alt"></i>
                                        </a>
                                        @can('manage-categories')
                                            <a href="javascript:void(0)"
                                               onclick="ativaDesativa('{{route('classifications.destroy',  $d->id)}}', 'desativar', '{{$d->label}}')"
                                    ...
                                    @else
                                        <a href="javascript:void(0)"
                                           onclick="ativaDesativa('{{route('classifications.destroy',  $d->id)}}', 'ativar', '{{$d->label}}')"
